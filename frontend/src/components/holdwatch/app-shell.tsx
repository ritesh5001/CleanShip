...
const NAV: Record<Session["role"], { href: string; label: string }[]> = {
  admin: [
    { href: "/holdwatch/admin", label: "Jobs" },
    { href: "/holdwatch/admin/clients", label: "Clients" },
    { href: "/holdwatch/admin/users", label: "People" },
    { href: "/admin", label: "Enquiries" },
  ],
  /* An editor works the enquiry inbox and has no Hold Watch surface; they can
     reach this shell only by URL, so it points them back rather than showing
     an empty nav. */
  editor: [{ href: "/admin", label: "Enquiries" }],
  supervisor: [{ href: "/holdwatch/app", label: "My jobs" }],
  client: [{ href: "/holdwatch/client", label: "Our jobs" }],
};
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white">
        ...
        >
          <Link href="/holdwatch" className="flex shrink-0 items-baseline gap-2">
          ...
          <form action="/holdwatch/logout" method="post" className="shrink-0">
            <button
              type="submit"
