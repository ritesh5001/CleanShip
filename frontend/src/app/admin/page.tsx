  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        ...
        {rows.length === 0 ? (
          <div className="mt-6 rounded-none border border-slate-200 bg-white p-10 text-center">
          ...
          <ul className="mt-6 space-y-3">
            {rows.map((e) => (
              <li
                key={e.id}
                className="rounded-none border border-slate-200 bg-white p-5"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <span
                      className={`inline-flex rounded-none border px-2.5 py-0.5 text-[12px] font-semibold capitalize ${STATUS_STYLE[e.status]}`}
                    >
                      {e.status.replace("-", " ")}
                    </span>
                    <p
                ...
                <form action={setEnquiryStatusAction} className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <select
                    ...
                    className="min-h-9 rounded-none border border-slate-300 px-2 text-[13px]"
                  >
                    {statuses.map((s) => (
                  ...
                  <button
                    type="submit"
                    className="min-h-9 rounded-none border border-slate-300 bg-white px-3 text-[13px] font-semibold text-slate-800 hover:bg-slate-50"
                  >
                    Save
                  </button>
                </form>
