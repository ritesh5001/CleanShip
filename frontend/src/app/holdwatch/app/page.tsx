...
import { EmptyState, PageTitle } from "@/components/holdwatch/ui";
import { getJobDetail, jobProgress, listJobsForSupervisor } from "@/lib/holdwatch/jobs";
import { InstallPrompt } from "@/components/holdwatch/install-prompt";
          ...
          <section className="space-y-3">
            {active.map(({ job, progress }) => (
              <JobCard
                ...
                job={job}
                progress={progress}
                href={`/holdwatch/app/jobs/${job.id}`}
              />
            ))}
          </section>
