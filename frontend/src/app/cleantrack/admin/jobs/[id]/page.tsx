...
export default async function AdminJobPage({
  ...
  return (
    <AppShell session={session} wide>
      <Link href="/cleantrack/admin" className="text-[13px] font-medium text-blue-700 hover:underline">
        ← All jobs
      </Link>
        ...
        <div className="space-y-5">
          ...
          <Card className="p-5">
            ...
            <p className="mt-2 text-[13px] text-slate-600">
              Read-only live view, no account needed. {job.imo
                ? "The customer enters the vessel's IMO number to open it."
                : "This vessel has no IMO on record, so the link opens directly — add an IMO above to gate it."}
            </p>
            {job.shareRevoked ? (
