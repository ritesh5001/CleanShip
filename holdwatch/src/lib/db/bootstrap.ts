  ...
  if (existingAdmins.length > 0) {
    console.error(
      `\nAn admin already exists (${existingAdmins[0].email}).\n` +
        "Create further accounts from /admin/users while signed in as that " +
        "account — this script only bootstraps an empty system.\n",
    );
    process.exit(1);
  }
...
Sign in, then add supervisors and clients from /admin/users.
`);
}
