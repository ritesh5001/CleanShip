...
import { getJobDetail, jobProgress, listAllJobs } from "@/lib/jobs";
  ...
  return (
    <AppShell session={session} wide>
      <PageTitle
        ...
        action={<LinkButton href="/admin/jobs/new">New job</LinkButton>}
      />
      ...
      {jobs.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            action={<LinkButton href="/admin/jobs/new">Create the first job</LinkButton>}
          />
        </div>
      ) : (
            ...
            <section>
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                {live.map(({ job, progress }) => (
                  <JobCard key={job.id} job={job} progress={progress} href={`/admin/jobs/${job.id}`} />
                ))}
              </div>
            </section>
          )}
