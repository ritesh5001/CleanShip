  ...
  return (
    <section className="relative isolate overflow-hidden py-28 lg:py-40">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(100%_80%_at_50%_0%,#0c514e_0%,#0a1e31_50%,#04121f_100%)]" />
        <div className="grid-lines mask-fade absolute inset-0 opacity-50" />
      </div>

      <div className="container-page text-center">
        <p className="font-display text-7xl text-aqua-400/30 sm:text-8xl">404</p>
          ...
          {serviceCategories.map((category) => (
            <li key={category.slug}>
              <Link
                ...
                className="inline-block rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-abyss-200 transition hover:border-aqua-400/40 hover:text-aqua-300"
              >
                {category.name}
              </Link>
            </li>
