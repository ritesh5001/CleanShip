 ...
 *
 * ⚠️ ALL OF THIS IS TEMPORARY STOCK — replace with CleanHull's own photographs.
 *
 ...
 *    that does not exist.
 *
 * 2. They are illustrative of the *operation*, not records of CleanHull jobs.
 *
...
export const heroImageByService: Record<string, StockImage> = {
  // Every hull scope has its own footage, so nothing needs a still fallback.
};
