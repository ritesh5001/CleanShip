 ...
 */
const FROM =
  process.env.RESEND_FROM_EMAIL ?? `CleanHull Website <website@cleanhull.co>`;
...
function acknowledgementEmail(e: Enquiry) {
  ...
  const html = shell(
    ...
    `Thank you, ${esc(e.name)}. Your enquiry has reached our dive desk. We respond within one working day, and sooner for vessels already in port.`,
    `${detail.some(([, v]) => v) ? rows(detail) : ""}
     ...
     <p style="margin:22px 0 0;font-size:14px;line-height:1.65;color:${BRAND.slate};">
       ...
       call the dive desk directly — it is manned 24/7.
     </p>
  ...
  const text = [
    `Thank you, ${e.name}.`,
    "",
    "Your enquiry has reached our dive desk. We respond within one working day,",
...
export async function sendEnquiryEmails(enquiry: Enquiry): Promise<void> {
  ...
  try {
    const { error: ackError } = await resend.emails.send({
      ...
      subject: "We have your enquiry — CleanHull Marine Services",
      html: ack.html,
      text: ack.text,
    });
    if (ackError) {
