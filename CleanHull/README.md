# CleanHull Marine Services — Website

Marketing site for CleanHull Marine Services FZE, built with Next.js 15 (App
...
1. **Set the real domain.** `siteConfig.url` in `src/lib/site.ts` is
   `https://www.cleanhull.co`. Canonicals, the sitemap and OG URLs all derive
   from it.
...
| Email | To | Purpose |
| --- | --- | --- |
| Notification | `ENQUIRY_TO_EMAIL` (default `ops@cleanhull.co`) | The enquiry, with `replyTo` set to the enquirer so replying just works |
...
The site implements the **CleanHull Design System** in `cleanhull design system/`.
